...
//######### HACE USO DEL SERVICIO WEB QUE ESTA PUBLICADO DE MANERA LOCAL ########		 
	...
	$datos=$cliente->contarUsuariosIna();	
	...
	$datosPag=$cliente->consultarUsuariosIna($inicioPag,$numRegistro);			
    $estado=1;
   
    //VERIFICA QUE LA VARIABLE ne TENGA VALOR PARA ELIMINAR AL USUARIO
    if (isset($_GET['ne'])){
       
       	$datosDel=$cliente->actUsuario($_GET['ne']);	
       	if((int)$datosDel[0]["ID"]!=0)
       	{
       	    echo '<script language="javascript">alert("Usuario activado correctamente")</script>';
                    ...
                    <?php
                      ...
                      {	          
                    	for($rr=0;$rr<count($datosPag);$rr++){	
                    		echo "<tr>";
                    		echo "<td bg-primary><a href='?op=activar-usuario&ne=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys btn-options' title='Activar' ><i class='fas fa-sync-alt'></i></a>";
                	  ...
                	  {		
                		...
                		{		
                			echo " <a href='?op=activar-usuario&pagina=$i'>$i</a> ";
                		}
                	  }
                	  echo "</center>";

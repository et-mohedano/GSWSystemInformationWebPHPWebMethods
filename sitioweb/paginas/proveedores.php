...
//OBITNE LA PÁGINA ACTUAL O A LA QUE SE LE HIZO CLIC
	...
	$datos=$cliente->contarProveedores();	
	...
	$datosPag=$cliente->consultarProveedores($inicioPag,$numRegistro);			
    $estado=1;
   
    //VERIFICA QUE LA VARIABLE ne TENGA VALOR PARA ELIMINAR AL USUARIO
    if (isset($_GET['ne'])){
       
       	$datosDel=$cliente->eliminarProveedor($_GET['ne']);	
              ...
              <form id="frmConexion" name="frmConexion" method="POST">
                    ...
                    <?php
                      ...
                      {	          
                    	for($rr=0;$rr<count($datosPag);$rr++){	
                    		echo "<tr>";
                    		echo "<td bg-primary><a href='?op=modificar-proveedor&cve=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys btn-options' title='Editar' ><i class='fa fa-pencil-square-o'></i></a>
                                <br/><a href='?op=proveedores&ne=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys' title='Eliminar' ><i class='fa fa-times-circle'></i></a></td>";
                	  ...
                	  {		
                		...
                		{		
                			echo " <a href='?op=proveedores&pagina=$i'>$i</a> ";
                		}
                	  }
                	  echo "</center>";
                ...
                <hr>
                  <div class="text-center">
                      <a href='?op=registrar-proveedores' class="btn btn-sys">Registrar proveedores&nbsp;</a>
                      <a href='?op=activar-proveedor' class="btn btn-sys">Activar proveedores&nbsp;</a>
                  </div>
                <hr>
        </div>
        <?php

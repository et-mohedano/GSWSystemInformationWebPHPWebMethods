...
//######### HACE USO DEL SERVICIO WEB QUE ESTA PUBLICADO DE MANERA LOCAL ########		 
	...
	$datos=$cliente->contarCategoriasIna();	
	...
	$datosPag=$cliente->consultarCategoriasIna($inicioPag,$numRegistro);			
    $estado=1;
   
    //VERIFICA QUE LA VARIABLE ne TENGA VALOR PARA ELIMINAR AL USUARIO
    if (isset($_GET['ne'])){
       
       	$datosDel=$cliente->actCategoria($_GET['ne'], $_GET['cat']);	
       	if((int)$datosDel[0]["ID"]!=0)
       	{
       	    echo '<script language="javascript">alert("Categoria activada correctamente")</script>';
                    ...
                    <?php
                      ...
                      {	          
                    	for($rr=0;$rr<count($datosPag);$rr++){	
                    		echo "<tr>";
                    		echo "<td bg-primary><a href='?op=activar-categoria&ne=".$datosPag[$rr]["Clave"]."&cat=".$datosPag[$rr]["Categoria"]."' class='btn btn-sys btn-options' title='Activar' ><i class='fas fa-sync-alt'></i></a>";
                	  ...
                	  {		
                		...
                		{		
                			echo " <a href='?op=activar-categoria&pagina=$i'>$i</a> ";
                		}
                	  }
                	  echo "</center>";
